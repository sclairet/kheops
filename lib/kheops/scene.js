...
kh.Light.prototype.setPos = function setPos(pos) {
	if ('pos' in this) {
		this.pos = pos;
		++this.stamp;
	}
...
kh.Light.prototype.updateShader = function updateShader(glContext, shader) {
	...
	if (this.kind == kh.Light.kind.ambient) {
		...
		var pName = 'pointLightColor' + this.number.toString();
		if (pName in shader.uniforms) {
			glContext.uniform3fv( shader.uniforms.pointLightColor, this.color);
		}

		var pName = 'pointLightDirection' + this.number.toString();
		if (pName in shader.uniforms) {
	      	var vLightingDirection = vec3.create();
	      	vec3.normalize( this.direction, vLightingDirection);
	      	vec3.scale( vLightingDirection, -1);
			glContext.uniform3fv( shader.uniforms[pName], vLightingDirection);
		}

		var pName = 'pointLightPosition' + this.number.toString();
		if (pName in shader.uniforms) {
			glContext.uniform3fv( shader.uniforms[pName], this.pos);
		}		
	}
};
...
kh.Light.createPoint = function createPoint(color, direction, pos, number) {
	var light = new kh.Light(kh.Light.kind.point, color, direction || [0.0, 0.0, -1.0], position || [0.0, 0.0, 0.0]);
	light.number = number;
	return light;
};
