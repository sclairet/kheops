...
kh.Scene.prototype.createDrawingContext = function createDrawingContext(name, attachAmbientLight, attachDirectionalLight) {
	var context = new kh.DrawingContext(name, this.gl, this.shaderMgr, this.textureMgr, this.drawingMode);
	if (attachAmbientLight) {
		context.attachLight(this.lights[0]); // attach ambient light
	}
	if (attachDirectionalLight) {
		context.attachLight(this.lights[1]); // attache directional light
	}
	this.drawingContexts.push(context);
	return context
};
