...
/*	Light object
*/
kh.Light = function Light( pos, direction, diffuseColor, specularColor) {
	this.pos = pos;
	this.direction = direction;
	this.diffuseColor = diffuseColor || [1.0, 1.0, 1.0];
	this.specularColor = specularColor || [0.5, 0.5, 0.5];
};
...
kh.Scene = function Scene( glContext, properties, options) {
    ...
    this.withTextures = true;
    if ('textures' in opts) {
		this.withTextures = (opts.textures == 'enabled');
	}
		...
		for (var shaderIter = 0, len = names.length ; shaderIter < len ; ++shaderIter) {
			...
			if (lShader != null) {
				...
				if ('directionalLightColor' in lShader.uniforms) {
					this.gl.uniform3fv( lShader.uniforms.directionalLightColor, this.light.diffuseColor);
				}

				if ('diffuseLightColor' in lShader.uniforms) {
					this.gl.uniform3fv( lShader.uniforms.diffuseLightColor, this.light.diffuseColor);
				}

				if ('specularLightColor' in lShader.uniforms) {
					this.gl.uniform3fv( lShader.uniforms.specularLightColor, this.light.specularColor);	
				}
