...
kh.Model = function Model(desc) {
    ...
    this.curSmoothingGroup = new kh.Model.SmoothingGroup(0);
...
kh.Model.SmoothingGroup = function SmoothingGroup(number) {
    this.number = number;
    this.faces = [];
};
...
kh.Model.prototype.readVertex = function readVertex(values) {
    this.vertices.push( [parseFloat(values[1]), parseFloat(values[2]), parseFloat(values[3])] );
};
...
kh.Model.prototype.readSmoothingGroup = function readSmoothingGroup(values) {
    if ((values[1] === 'off') || (values[1] === '0')) {
         this.curSmoothingGroup = this.smoothingGroups[0];
    }
    else {
        var groupNumber = parseInt(values[1], 10);
        if (typeof(this.smoothingGroups[groupNumber]) == 'undefined') {
            this.curSmoothingGroup = new kh.Model.SmoothingGroup(groupNumber);
            this.smoothingGroups[groupNumber] = this.curSmoothingGroup;
        }
        else {
            this.curSmoothingGroup = this.smoothingGroups[groupNumber];
        }
    }
};
...
kh.Model.prototype.readMaterial = function readMaterial(values) {
    ...
    if (!(materialName in this.materialGroups)) {
        var material = new kh.Model.MaterialGroup(materialName);
        this.materialGroups[materialName] = material;
        this.curMaterialGroup = material;
    }
    else {
        this.curMaterialGroup = this.materialGroups[materialName];
    }   
};
...
kh.Model.prototype.readGroup = function readGroup(values) {
    ...
    for (var groupIter = 1 ; groupIter < values.length ; ++groupIter) {
        ...
        if (!(groupName in this.groups)) {
            ...
            this.curGroups.push(group);
        }
        else {
            this.curGroups.push(this.groups[groupName]);
        }
    }

    if (this.curGroups.length === 0) {
        this.curGroups.push( this.groups['default']);
    }
};
...
kh.Model.prototype.parse = function parse(text) {
    ...
    for (var lineIndex = 0, lineCount = lines.length ; lineIndex < lineCount ; ++lineIndex) {
        ...
        if (array[0] == "g") {
            // group
            this.readGroup(array);
        }
        else if (array[0] == 's') {
            // smoothing group
            this.readSmoothingGroup(array);
        }
        else if (array[0] == 'usemtl') {
            // material
            this.readMaterial(array);
        }
        else if (array[0] == "v") {
            // vertex
