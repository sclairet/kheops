...
kh.modelManager.prototype.loadModel = function loadModel(scene, desc, onLoadModelHandler) {
    ...
    if (modelState != null) {
        ...
        var getOnQueryModelHandler = function getOnQueryModelHandler(scene, modelState) {
            ...
            return function (requestProgressEvent) {
                ...
                if (request.readyState == 4) {
                    ...
                    modelState.onLoadHandlers.forEach(function(handler) {
                        var onBeforeAction = function (handler, model) {
                            return function (scene) {
                                handler(model);
                            };
                        };
                        scene.beforeRunActions.push(onBeforeAction(handler, modelState.model));
                    } );
                }
            };
        };
        ...
        if ('mtl' in desc) {
            ...
            var getLoadMaterialHandler = function getLoadMaterialHandler(scene, desc) {
               return function (name, materials) {
                    desc.materials = desc.materials || {};
                    for(iter in materials) {
                        var material = materials[iter];
                        desc.materials[iter] = material;
                        if(scene.withTextures && ('textureMap' in material)) {
                            if (('texturesMap' in desc) && (material.textureMap in desc.texturesMap)) {
                                material.texture = scene.textureMgr.loadTexture(desc.texturesMap[material.textureMap], false);
                            }
                            else {
                                material.texture = scene.textureMgr.loadTexture(material.textureMap, false);
                            }
                        }
                    };
                };
            ...
            scene.materialMgr.loadMaterials(desc.name, desc.mtl, 'MTL', getLoadMaterialHandler(scene, desc));
        }

        this.models.push(modelState);
        var req = new XMLHttpRequest();
        req.onreadystatechange = getOnQueryModelHandler(scene, modelState).bind(this);
...
kh.Model.getFlattenedIndexes = function getFlattenedIndexes(faces, drawingMode, vertices) {
    ...
    for (var faceIter = 0, faceLen = faces.length ; faceIter < faceLen ; ++faceIter) {
        ...
        var withNormals = face.normalIndex.length  > 0;
        var withTexture = face.textureIndex.length > 0;
        ...
        if (drawingMode == kh.kDrawingMode.kLines) {
            ...
            for (var triangleIter = 1, trianglesCount = face.vertexIndex.length - 2 ; triangleIter <= trianglesCount ; ++triangleIter)
            {
                ...
                if (withNormals) {
                    indexes.normal.push(face.normalIndex[0]);
                    indexes.normal.push(face.normalIndex[triangleIter]);
                    indexes.normal.push(face.normalIndex[triangleIter+1]);
                }
                if (withTexture) {
                    indexes.texture.push(face.textureIndex[0]);
                    indexes.texture.push(face.textureIndex[triangleIter]);
...
kh.Model.getFlattenedTextureCoords = function getFlattenedTextureCoords(textureCoords) {
    var flatTextureCoords = [];
    textureCoords.forEach(function(coords) {
        flatTextureCoords.push(coords[0]);
        flatTextureCoords.push(coords[1]);
    } );
    return flatTextureCoords;
};
...
kh.Model.Face.prototype.getFlattenedIndexes = function getFlattenedIndexes(drawingMode, vertices) {
    ...
    var withNormals = this.normalIndex.length > 0;
    var withTexture = this.textureIndex.length > 0;
    ...
    if (drawingMode == kh.kDrawingMode.kLines) {
        ...
        for (var triangleIter = 1, trianglesCount = this.vertexIndex.length - 2 ; triangleIter <= trianglesCount ; ++triangleIter)
        {
            ...
            if (withNormals) {
                indexes.normal.push(this.normalIndex[0]);
                indexes.normal.push(this.normalIndex[triangleIter]);
                indexes.normal.push(this.normalIndex[triangleIter+1]);
            }
            if (withTexture) {
                indexes.texture.push(this.textureIndex[0]);
                indexes.texture.push(this.textureIndex[triangleIter]);
...
kh.Model.prototype.readFace = function readFace(values) {
    ...
    // sanity check
    if (    ((face.textureIndex.length > 0) && (face.textureIndex.length != face.vertexIndex.length))
        ||  ((face.normalIndex.length > 0) && (face.normalIndex.length != face.vertexIndex.length)) ) {
        console.error('inconsistant face found: ' + values.toString());
    }
...
kh.Model.prototype.parse = function parse(text) {
    ...
    console.log(this.desc.name + 'model vertices count:' + this.vertices.length);
    console.log(this.desc.name + 'model polygons count:' + this.faces.length);
    console.log(this.desc.name + 'model smoothing groups count: ' + this.smoothingGroups.length);
    console.log(this.desc.name + 'model materials count: ' + this.materialNames.length);
};
                    ...
                    for (var faceIter = 0, faceLen = sg.faces.length ; faceIter < faceLen ; ++faceIter) {
                        ...
                        for (var indexIter = 0, indexLen = fIndexes.vertex.length ; indexIter < indexLen ; ++indexIter) {
                            ...
                            if (typeof(pIndexMap[index]) == 'undefined') {
                                ...
                                if (typeof(fIndexes.texture[indexIter]) != 'undefined') {
                                    var textureIndex = fIndexes.texture[indexIter];
                                    if (typeof(this.textureCoords[textureIndex]) != 'undefined') {
                                        pTextureCoords[globalIndex] = this.textureCoords[textureIndex].slice();
                                    }
                                }
		...
		var getOnLoadModelHandler = function getOnLoadModelHandler( scene, properties, onCreatePrimitiveHandler) {
			...
			return function (model) {
                ...
                if (!withMaterials) {
                    ...
                    // create one primitive per group
                    var primitives = [];
                    ...
                    model.materialNames.forEach(function(name) {
                        ...
                        var pProperties = {
                            ...
                            'normals': kh.vectors3Array.create(),
                            'indexes': [],
                            'textureCoords': []
                        };
                        ...
                        pProperties.material =  (('materials' in model.desc) && (group.name in model.desc.materials)) 
                                        ? model.desc.materials[group.name]
                                        : kh.materials.desc.default;

                        var withTexture = 'texture' in pProperties.material;

                        pProperties.color =     (('colorsMap' in model.desc) && (group.name in model.desc.colorsMap))
                        ...
                        group.indexes.forEach(function(index) {
                            ...
                            if (typeof(pIndexMap[index]) == 'undefined') {
                                pIndexMap[index] = pGlobalIndex;
                                pProperties.indexes.push(pGlobalIndex);
                                pProperties.vertices[pGlobalIndex] = components.vertices[index].slice();
                                pProperties.normals[pGlobalIndex] = components.normals[index].slice();
                                if (withTexture) {
                                    if (typeof(components.textureCoords[index]) != 'undefined') {
                                        textureCoords[pGlobalIndex] = components.textureCoords[index].slice();
                                    }
                                    else {
                                        console.log('missing texture coordinates at index ' + index);
                                        textureCoords[pGlobalIndex] = [0.0, 0.0];
                                    }
                                }
                                ++pGlobalIndex;
                            }
                            else {
                                pProperties.indexes.push(pIndexMap[index]);
                            }
                        } );

                        if (withTexture) {
                            pProperties.textureCoords = kh.Model.getFlattenedTextureCoords(textureCoords);
                        }

                        var primitive = new kh.Primitive(scene, pProperties, kh.primitive.model);
                        primitive.desc = model.desc;
                        primitive.materialName = name;
                        primitives.push(primitive);
                    } );

                    onCreatePrimitiveHandler(primitives);
                }
			};
		};

    	scene.modelMgr.loadModel(scene, desc, getOnLoadModelHandler( scene, properties, onCreatePrimitiveHandler));
     },

	'draw': function draw( mvMatrix, drawingContext) {

        var gl = drawingContext.gl;
        var isTransparent = (this.material.transparency < 1.0);
