...
kh.Model = function Model(desc, text) {
    ...
    this.curSmoothingGroup = new kh.Model.SmoothingGroup(0, desc.forceOneSmoothingGroup);
...
kh.Model.prototype.getPrimitiveComponents = function getPrimitiveComponents(options) {
    ...
    var modelDrawMode = ('modelDrawMode' in options) ? options.modelDrawMode : kh.kModelDrawMode.kDisableGroups;
    var drawingMode = ('drawingMode' in options) ? options.drawingMode : kh.kDrawingMode.kTriangles;

    if (modelDrawMode == kh.kModelDrawMode.kDisableGroups) {

        var smoothingMode = ('smoothingMode' in options) ? options.smoothingMode : kh.kSmoothingMode.kForced;
        if (smoothingMode == kh.kSmoothingMode.kDisabled) {
        ...
        else if (smoothingMode == kh.kSmoothingMode.kForced) {
        ...
        var smoothingMode = ('smoothingMode' in options) ? options.smoothingMode : kh.kSmoothingMode.kPerGroup;
        var globalIndex = 0;
        ...
        for (var sgIter = 0, sgLen = this.smoothingGroups.length ; sgIter < sgLen ; ++sgIter) {
            ...
            if ((!sg.isSmooth && (smoothingMode == kh.kSmoothingMode.kPerGroup)) || (smoothingMode == kh.kSmoothingMode.kDisabled)) {
            ...
            else if ((sg.isSmooth && (smoothingMode == kh.kSmoothingMode.kPerGroup)) || (smoothingMode == kh.kSmoothingMode.kForced)) {

                var pIndexMap = [];
                var gIndexes = kh.Model.getFlattenedIndexes(sg.faces, drawingMode, this.vertices);
                var facesPerVertex = kh.Model.getFacesPerVertex(sg.faces);
                for (var indexIter = 0, indexLen = gIndexes.vertex.length ; indexIter < indexLen ; ++indexIter) {

                    var index = gIndexes.vertex[indexIter];
                    if (typeof(pIndexMap[index]) == 'undefined') {
                        pIndexMap[index] = globalIndex;
                        pIndexes.push(globalIndex);
                        pVertices[globalIndex] = this.vertices[index].slice();
                        ...
                        var normalDone = false
                        if (smoothingMode != kh.kSmoothingMode.kForced) {
                            if (typeof(gIndexes.normal[indexIter]) != 'undefined') {
                                var normalIndex = gIndexes.normal[indexIter];
                                if (typeof(this.normals[normalIndex]) != 'undefined') {
                                    pNormals[globalIndex] = this.normals[normalIndex].slice();
                                    normalDone = true;
                                }
...
/* 
    parse OBJ file content and returns model structure
*/
kh.createModel = function createModel( text, desc, properties)
{
    var mod = new kh.Model(desc, text);
    var components = mod.getPrimitiveComponents(properties.params);
...
kh.modelManager.prototype.loadModel = function loadModel(desc, properties, onLoadHandler) {
    ...
    if (model != null) {
        ...
        var getOnLoadObjectHandler = function getOnLoadObjectHandler(desc, properties, onLoadHandler) {
            ...
            return function (requestProgressEvent) {
                ...
                if (request.readyState == 4) {
                    ...
                    model = kh.createModel(request.responseText, desc, properties);
                    if (model !== null) {
        ...
        req.onreadystatechange = getOnLoadObjectHandler(desc, properties, onLoadHandler).bind(this);
...
kh.primitive.model = {
     ...
     'create': function create( scene, properties, desc, onCreateHandler) {
    	...
    	scene.modelMgr.loadModel(desc, properties, getOnLoadModelHandler( scene, properties, onCreateHandler));
     },
