...
kh.Model.prototype.getPrimitiveComponents = function getPrimitiveComponents(options) {
    ...
    var pIndexes = [];
    var pIndexesPerMaterial = [];

    for(mat in this.materialGroups) {
        pIndexesPerMaterial[mat] = {
            'material': mat,
            'indexes': []
        };
    }

    var drawingMode = kh.kDrawingMode.nameToValue(lOpts.drawingMode);
    ...
    if (lOpts.smoothingGroups == 'disabled') {
        ...
        if (smoothingMode == kh.kModelSmoothingMode.kDisableSmoothing) {
            ...
            for (var faceIter = 0, faceLen = this.faces.length ; faceIter < faceLen ; ++faceIter) {
                ...
                var fIndexes = face.getFlattenedIndexes(drawingMode, this.vertices).vertex;
            ...
            pIndexes = kh.Model.getFlattenedIndexes(this.faces, drawingMode, this.vertices).vertex;
            ...
            if (typeof(sg) != 'undefined') {
                if ((!sg.isSmooth && (smoothingMode == kh.kModelSmoothingMode.kPerGroup)) || (smoothingMode == kh.kModelSmoothingMode.kDisableSmoothing)) {
                    ...
                    for (var faceIter = 0, faceLen = sg.faces.length ; faceIter < faceLen ; ++faceIter) {
                        ...
                        var fMaterialIndexes = pIndexesPerMaterial[face.material].indexes;

                        var fIndexes = face.getFlattenedIndexes(drawingMode, this.vertices);
    ...
    return {
        ...
        'normals': pNormals,
        'indexes': pIndexes,
        'indexesPerMaterial': pIndexesPerMaterial,
...
kh.primitive.model = {
     ...
     'create': function create( scene, properties, desc, onCreatePrimitiveHandler) {
		...
		var getOnLoadModelHandler = function getOnLoadModelHandler( scene, properties, onCreatePrimitiveHandler) {
			...
			return function (model) {
                ...
                primitive.drawingMode = components.drawingMode;

                if (withMaterials) {
                    primitive.indexesPerMaterial = [];
                    for(material in components.indexesPerMaterial) {
                        if ('material' in components.indexesPerMaterial[material]) {
                            primitive.indexesPerMaterial.push( {
                                'material': components.indexesPerMaterial[material].material,
                                'vertexIndexBuffer': kh.createVertexIndexBuffer(scene.gl, components.indexesPerMaterial[material].indexes)
                            } );
                        }
                    }
	...
	'draw': function draw( mvMatrix, drawingContext) {
        ...
        if (shader != null) {
            if ('indexesPerMaterial' in this) {
                for (var iter = 0, len = this.indexesPerMaterial.length ; iter < len ; ++iter) {

                    gl.bindBuffer( gl.ELEMENT_ARRAY_BUFFER, this.indexesPerMaterial[iter].vertexIndexBuffer);

                    if ('colorsMap' in this.desc) {
                        if (this.indexesPerMaterial[iter].material in this.desc.colorsMap) {
                            var color = this.desc.colorsMap[this.indexesPerMaterial[iter].material];
                            gl.uniform4f( shader.uniforms.uniformColor, color[0], color[1], color[2], color[3]);
                        }
                    }

                    if (this.indexesPerMaterial[iter].material in this.desc.materials) {
                        var mat = this.desc.materials[this.indexesPerMaterial[iter].material];
                        gl.uniform3fv( shader.uniforms.ambientLightWeight, mat.ambientLightWeight);
                        gl.uniform3fv( shader.uniforms.diffuseLightWeight, mat.diffuseLightWeight);
                        gl.uniform3fv( shader.uniforms.specularLightWeight, mat.specularLightWeight);
                        gl.uniform1f( shader.uniforms.shininess, mat.shininess);
                        gl.uniform1f( shader.uniforms.transparency, mat.transparency);
                    }

                    if (this.drawingMode == kh.kDrawingMode.kLines) {
                        gl.drawElements(gl.LINES, this.indexesPerMaterial[iter].vertexIndexBuffer.numItems, gl.UNSIGNED_SHORT, 0);
                    }
                    else if (this.drawingMode == kh.kDrawingMode.kTriangles) {
                        gl.drawElements(gl.TRIANGLES, this.indexesPerMaterial[iter].vertexIndexBuffer.numItems, gl.UNSIGNED_SHORT, 0);
                    }
                }
            }
            else {
