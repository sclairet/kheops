...
kh.defaultValues.defaultSizes = [kh.defaultValues.with, kh.defaultValues.height, kh.defaultValues.depth];
...
kh.getArcVertices = function getArcVertices(properties) {
    ...
    for (var vertexIter = 0 ; vertexIter < (segPerArc + 1) ; ++vertexIter) {
        vertices.push([
            hRadius * Math.cos(angle.min + (vertexIter * angleStep)),
            vRadius * Math.sin(angle.min + (vertexIter * angleStep)),
            0.0
        ]);
    }
