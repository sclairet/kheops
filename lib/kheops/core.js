...
Array.prototype.mat4Push= function mat4Push(matrix) {
    for (var iter = 0 ; iter < 16 ; ++iter) {
        this.push(matrix[iter]);
    }
};


Array.prototype.mat4Set= function mat4Set(index, matrix) {
    for (var iter = (index * 16), valueIter = 0 ; iter < ((index + 1) * 16) ; ++iter, ++valueIter) {
        this[iter] = matrix[valueIter];
    }
};
...
Array.prototype.clone = function clone() {
    var c = [];
    this.forEach(function(value) {c.push(value);})
    return c;
};
...
kh.ShiftList.prototype.forEach = function forEach(functor) {
    this.mList.forEach(function(item, index, array) {
        functor(item, index, array);
    });
};
