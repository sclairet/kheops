...
kh.obj.cylinder = {
	...
	'createWithDesc': function createWithDesc( scene, properties) {
		var props = properties || {};
		var obj = new kh.Obj( scene, props);
		if (('independantFaces' in props) && props.independantFaces) {
			var desc = kh.primitive.cylinder.createDescriptor(props);
			obj.primitives.push(kh.primitive.square.createWithDesc(scene, desc.middle));
			obj.primitives.push(kh.primitive.disc.createWithDesc(scene, desc.top));
			obj.primitives.push(kh.primitive.disc.createWithDesc(scene, desc.bottom));
		}
		else {
			obj.primitives.push( kh.primitive.cylinder.createWithDesc( scene, properties));
		}
		obj.kind = 'cylinder';
		return obj;
	}
